...
func (drv *Driver) CreateTable(tabName string, fieldSpecs []dna.FieldSpec) error {
	...
	for _, fieldSpec := range fieldSpecs {
		...
		} else if len(fieldSpec.Fk) > 0 {
			colNames += " NUMBER"
		} else {
			colType, err := oracleTypeFromFieldSpec(fieldSpec)
			if err != nil {
				return err
			}
			colNames += " " + colType
	...
	sql := fmt.Sprintf(`CREATE TABLE `+qt+`%s`+qt+` (%s%s)`, tabName, colNames, pkConstraint)
	Goose.Init.Logf(5, sql)
...
func oracleTypeFromFieldSpec(f dna.FieldSpec) (string, error) {
	var prec, frac int
